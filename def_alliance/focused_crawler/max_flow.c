...
struct vertex * * source_component_min_cut_after_max_flow(struct graph * g, struct vertex * source, struct vertex* target, uint64_t k, uint64_t * source_component_size)
{
	...
	struct vertex** result = malloc((size_t)size * sizeof(struct vertex*));
	if (result == NULL)
		exit(-1);
	struct edge * e;
	struct vertex * v;
	...
	while (v_q->first)
	{
		...
		{
			size *= 2;
			struct vertex** tmp = realloc(result, (size_t)size * sizeof(struct vertex*));
			if (tmp == NULL)
				exit(-1);
			result = tmp;
		}
	...
	struct vertex** tmp = realloc(result, (size_t)index * sizeof(struct vertex*));
	if (tmp == NULL)
		exit(-1);
	result = tmp;
